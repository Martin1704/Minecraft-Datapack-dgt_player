tellraw @a[tag=debug] {"text":"debug: ","color":"red","extra":[{"text":"chk_uuid_set"}]}
...
execute if data storage dgt_player:temp check{UUIDDiff:1} if data storage dgt_db:main return.UUIDs[0] run function dgt_player:player_id/chk_uuid_set
